...
flags.DEFINE_string('dataset_dir', './datasets/CIHP/tfrecord', 'Where the dataset reside.')
    ...
    with tf.Graph().as_default() as graph:
        with tf.device(config.inputs_device()):
            dataset = data_generator.Dataset(
                ...
                should_shuffle=True,
                should_repeat=True,
                has_rev=True if FLAGS.dataset == 'cihp' else False)
        ...
        config.allow_growth = True  # dynamically grow the memory used on the GPU
...
if __name__ == '__main__':
    ...
    FLAGS.initialize_last_layer = False
    FLAGS.train_batch_size = 2
    FLAGS.model_variant = 'mobilenet_v2'
    FLAGS.base_learning_rate = 1e-5
    FLAGS.training_number_of_steps = 5000
