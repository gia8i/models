...
def flip_dim_with_rev(tensor_list, prob=0.5, dim=1):
    """Randomly flips a dimension of the given tensor.
    ...
    def flip():
        ...
        for tensor in tensor_list[:1]:
            if dim < 0 or dim >= len(tensor.get_shape().as_list()):
                raise ValueError('dim must represent a valid dimension.')
            flipped.append(tf.reverse_v2(tensor, [dim]))
        flipped.append(tensor_list[-1])
        return flipped

    is_flipped = tf.less_equal(random_value, prob)
    outputs = tf.cond(is_flipped, flip, lambda: tensor_list[:-1])
    if not isinstance(outputs, (list, tuple)):
        outputs = [outputs]
    outputs.append(is_flipped)
...
def randomly_scale_image_and_label(image, label=None, scale=1.0, label_rev=None):
    ...
    Args:
      ...
      label_rev: Label rev with shape [height, width, 1]
    ...
    if label_rev is not None:
        label_rev = tf.image.resize(
            label_rev,
            new_dim,
            method=get_label_resize_method(label_rev),
            align_corners=True)
...
def resize_to_range(image,
    ...
    Args:
      ...
      label_rev: label rev used in body parsing segmentation with left right labels
    ...
    with tf.name_scope(scope, 'resize_to_range', [image]):
        ...
        if label_rev is not None:
            if label_layout_is_chw:
                # Input label_rev has shape [channel, height, width].
                resized_label_rev = tf.expand_dims(label_rev, 3)
                resized_label_rev = tf.image.resize(
                    resized_label_rev,
                    new_size,
                    method=get_label_resize_method(label_rev),
                    align_corners=align_corners)
                resized_label_rev = tf.squeeze(resized_label_rev, 3)
            else:
                # Input label_rev has shape [height, width, channel].
                resized_label_rev = tf.image.resize(
                    label_rev,
                    new_size,
                    method=get_label_resize_method(label_rev),
                    align_corners=align_corners)
            new_tensor_list.append(resized_label_rev)
        else:
            new_tensor_list.append(None)
