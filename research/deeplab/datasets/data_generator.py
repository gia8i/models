...
_CIHP_INFORMATION = DatasetDescriptor(
    splits_to_sizes={
        'train': 26866,  # num of samples in images/training
        'val': 1414,  # num of samples in images/validation
    },
    num_classes=20,
    ignore_label=0,
)
...
class Dataset(object):
    ...
    def _parse_function(self, example_proto):
        ...
        if self.has_rev:
            features['image/segmentation/class/encoded_rev'] = \
                tf.FixedLenFeature((), tf.string, default_value='')
        ...
        if self.split_name != common.TEST_SET:
            ...
            if self.has_rev:
                label_rev = _decode_image(
                    parsed_features['image/segmentation/class/encoded_rev'], channels=1)
        ...
        if label_rev is not None:
            if label_rev.get_shape().ndims == 2:
                label_rev = tf.expand_dims(label, 2)
            elif label_rev.get_shape().ndims == 3 and label_rev.shape.dims[2] == 1:
                pass
            else:
                raise ValueError('Input label rev shape must be [height, width], or '
                                 '[height, width, 1].')

            label_rev.set_shape([None, None, 1])

            sample[common.LABELS_REV_CLASS] = label_rev
    ...
    def _preprocess_image(self, sample):
        ...
        if self.has_rev:
            label_rev = sample[common.LABELS_REV_CLASS]
        else:
            label_rev = None
